...
| 02-08-T2 | `packages/mcp-server/src/__tests__/integration/recall-conflicts.test.ts` — 4-case CON-05 integration test | claude | N/N/Y | Q1=N (single-file test); Q2=N (~280 LOC but behavioral test design requires judgment — malformed-JSON spy setup, DO stub proxy pattern, workspace seeding logic not mechanical); Q3=Y (cross-workspace-pentest.test.ts + tools-integration.test.ts are structural templates). Direct claude — Proxy wrapper on DO stub requires reasoning about workerd stub interface. | n/a |
| 02-08-T3 | `packages/mcp-server/src/__tests__/no-proactive-notifications.test.ts` — CON-08 grep gate | claude | N/N/Y | Q1=N (single-file test); Q2=N (~40 LOC, below 50-line savings threshold); Q3=Y (`lint-no-direct-vectorize.test.ts` is the byte-frozen template). Small diff, context-prep overhead exceeds savings. Direct claude. | n/a |
| 02-07-T1a | `packages/triage-worker/src/index.ts` — `ctx` param + import + `ctx.waitUntil(conflictPipeline(...))` insertion (~15 LOC) | claude | N/N/Y | Q1=N (single-file surgical); Q2=N (<15 lines changed — import, signature, 12-line insertion block); Q3=Y (mcp-server `ctx.waitUntil(env.INGEST_QUEUE.send(...))` is the stable fire-and-forget template). Below 15-line threshold — context-prep overhead exceeds savings. | n/a |
| 02-07-T1b | `packages/triage-worker/src/__tests__/queue-integration.test.ts` — CON-03 wiring tests (3 new cases + buildCtx helper + vi.mock + ctx arg threading ~80 LOC net new) | claude | N/Y/Y | Q1=N (single-file test); Q2=Y (~80 LOC net new: buildCtx helper + vi.mock + 3 test cases + 10 call-site ctx arg adds); Q3=Y (existing queue-integration.test.ts spy patterns are the structural template). Eligible for cf-code-assist, but test design requires behavioral judgment about waitUntil + vi.mocked patterns + assertion of "no embedding" property — reasoned choice to keep with claude. | n/a |
| 02-09-T1 | `scripts/eval-budget-summary.mjs` — `--conflict-pipeline-p99` mode (~100 LOC additive: arg-parse, mode dispatch, `runConflictPipelineP99Mode()` with SQL POST + response parse + 4 exit codes) | claude | N/Y/Y | Q1=N (single-file, no cross-file synthesis — new function appended to existing script); Q2=Y (~100 LOC mechanical extension); Q3=Y (existing eval-budget-summary.mjs nightly-summary mode is the stable template; SQL query pre-pinned verbatim in plan interfaces; D-20 schema confirmed from conflict-pipeline.ts). Eligible for cf-code-assist; tool surface unavailable in executor subagent — routed to claude and recorded honestly. | n/a |
