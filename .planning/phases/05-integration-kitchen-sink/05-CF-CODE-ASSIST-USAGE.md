...
| 05-03-T2 (cross-workspace-pentest, pre-classified) | `packages/mcp-server/src/__tests__/cross-workspace-pentest.test.ts` additions (3 Prong-A + 3 Prong-C stubs, ~80 LOC) | `cf-code-assist:scaffoldTests` | N/Y/Y | Single-file additions >50 LOC, stable Prong-A / Prong-C template from existing file — N/Y/Y candidate. **(pre-classified — update at execution time)** | ~2–4K tokens |
