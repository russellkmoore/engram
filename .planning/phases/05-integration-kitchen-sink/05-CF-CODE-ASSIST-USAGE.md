...
| 05-04-T1 (actual) | `packages/triage-worker/src/__tests__/conflict-pipeline-isolation.test.ts` (new file, ~175 LOC, makeWorkspaceStub + D-10 isolation assertion) | `claude` | N/Y/Y | Q1=No (single-file new test, no cross-file consistency invariants). Q2=Yes (~175 LOC including fixtures + 2 test cases). Q3=Yes (stable makeWorkspaceStub template + D-10 assertion spec from PATTERNS.md). **Route decision: claude** — binding constraint is runtime-GREEN iteration. cf-code-assist cannot observe test failures. The D-10 isolation logic requires exact mock wiring (idFromName.mockImplementation vs mockReturnValue, correct score threshold for prefilter gate) that only validates at runtime. The pre-classified estimate suggested cf-code-assist, but the isolation logic correctness is verifiable only by running the test — same pattern as 05-02-T1 and 05-03-T2. Test passed GREEN on first write attempt. | n/a |
