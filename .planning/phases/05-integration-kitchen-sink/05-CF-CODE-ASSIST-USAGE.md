...
| 05-02-T1 (pre-classified estimate) | `packages/mcp-server/src/__tests__/integration/v02-kitchen-sink.test.ts` body generation (~200+ LOC test scaffold with known assertion spec from PATTERNS.md) | `cf-code-assist:scaffoldTests` | N/Y/Y | Single-file mechanical test scaffold >200 LOC, stable PATTERNS.md + RESEARCH.md assertion spec available — N/Y/Y candidate. **(pre-classified estimate — update at execution time)** | ~6–9K tokens |
| 05-03-T1 (pre-classified estimate) | `packages/mcp-server/src/__tests__/integration/cross-workspace-pentest.test.ts` additions (~60 LOC, 3 Prong-A blocks, Prong C skip pattern well-defined) | `cf-code-assist:scaffoldTests` | N/Y/Y | Single-file additions >50 LOC, stable Prong-A / Prong-C template from existing file — N/Y/Y candidate. **(pre-classified estimate — update at execution time)** | ~2–4K tokens |
