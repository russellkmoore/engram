...
| 04-03-T1 | `trimRankedForSynthesis` (SYN-05 throw + export) + `applyHedgePrefix` + `mapPositionsToCitationIds` + `dropUncitedSentences` in `tools.ts` | claude | Y/N/Y | SYN-05 throw must coordinate with the synthesis try/catch catch-block (same file, but the throw propagation is a cross-function consistency invariant — the throw must be caught by the EXISTING try/catch to maintain honest-stubs posture); stable PATTERNS.md/RESEARCH.md spec available. | n/a |
| 04-03-T2 | Synthesis block guards + post-processor chain + SYN-09 analytics wiring in `tools.ts` | claude | Y/N/Y | Byte-frozen SYNTHESIS_SYSTEM_PROMPT adjacency (D-01/SYN-10); writeAnalytics blob layout must match across both call sites simultaneously (cross-site consistency invariant); meta.gaps pattern must match existing queryWasTruncated/expansionUnavailable channels (EXP-10). Multi-invariant coordination qualifies for Claude. | n/a |
| 04-04-T1 | `packages/mcp-server/src/__tests__/evals/synthesis-fidelity.eval.test.ts` (~300 LOC eval file) | claude (fallback) | N/Y/Y | Planned route: `cf-code-assist:scaffoldTests` (N/Y/Y — single-file, >50 LOC, recall-latency.eval.test.ts template analog + judge-call spec). Actual: scaffoldTests tool lacks a context-injection channel suitable for verbatim-helper-copy requirements (computePercentile/hasEvalCreds must be copied byte-for-byte from recall-latency.eval.test.ts; JUDGE_MODEL wiring + zod v4 z.toJSONSchema() fix require project-specific knowledge); endpoint also returned AI_TIMEOUT x2 this session. Claude generated directly from template files. Route-audit: intended ~5–8K tokens saved. | n/a (intended ~5–8K tokens) |
| 04-04-GAP1 | `tools.ts` generateSynthesis() extraction + `synthesis-fidelity.eval.test.ts` direct-helper rewrite | claude | Y/N/Y | Behavior-preserving tools.ts extraction + eval rewrite; runtime-debug + cross-function invariant (throw propagation from generateSynthesis into recall()'s synthesisGaps merge). | n/a |
